...
// --------------------------------------------------
//
// A JavaScript layer to enhance the usability of the ILLiad frontend interface
//
// Updated:     29 June 2016
// Creators:    BYU Harold B. Lee Library (Grant Zabriskie, Joe Larson)
// Maintainers: BYU Harold B. Lee Library (Ben Crowder)
//
// Requires:    jQuery
//
// The BYU Harold B. Lee Library cannot provide support for ILLiadPlus.js or 
// push upgrades to support other instances of ILLiad.
...
function illplus_requests() {
	...
	$(document).ready(function() {
		...
		function getNewStatus(status) {
			...
			// Also trim any whitespace or blank characters (needed for downloadable items below)
			...
			switch (cleanStatus) {
				...
				case 'Customer Notified via E-Mail':
					newStatus = 'Item Ready for Pick-up at Default Location';
					break;
				...
				case '': // Downloadable items don't have visible statuses through the tables
				...
				case "Awaiting OCLC Sending" :
                case "Awaiting Further Searching" :
                case "Awaiting Extensive Searching" :
				case "Request Sent" :
                case "Awaiting RAPID Local Sending" :
                case "Awaiting RAPID Request Sending" :
                case "Awaiting Purchase on Demand Processing" :
                case "Awaiting OWEN RAPID Processing" :
                case "Potential Purchasing - Access Services" :newStatus = "Making Inquiries";
					break;
				case "Awaiting Document Delivery Processing" :
                case "Awaiting Delivery from AS Purchase" :
                case "Awaiting Purchase on Demand Processing" :
                case "Awaiting Response from Email Inquiry" :
                case "Awaiting Resubmission to OCLC" :
                case "Awaiting Unfilled Processing" :
                ...
                case "Sue's Queue" :
                case "Sean's Queue" :
                case "Owen DocDel Queue" :
                case "Owen Borrowing Queue" :
                case "HaT Borrowing Queue" :
                case "Caty's Queue" :
                case "AHL Doc Del Queue" :
                case "Awaiting HaT Doc Del Processing" :
                case "Awaiting Online Delivery" :
                case "Owen Doc Del Queue" :
		...
		function getSection(newStatus) {
			...
			var checkedOutStatusList = new Array('On Its Way', 'Renewal in process');
			var availableStatusList = new Array('Delivered to Web', 'Available to download');
			var inProcessStatusList = new Array('Awaiting processing', 'In processing', 'In process');
		...
		// Return string of option links associated with item
		// status and id
		// --------------------------------------------------
		...
		function getOptions(newStatus, id) {
			...
			switch (newStatus) {
				...
				case 'Available for download':
					result += view + ' | ' + download + ' | ' + deleteDownload;
					break;

				case 'Cancelled':
					result += view + ' | ' + resubmit;
					break;
			}
		...
		function appendSection(section) {
			...
			for (i=0; i<records.length; i++) {
				if (records[i].section == section) {
					...
					result += '<div class="ill_request_options">';
					result += '<span class="ill_request_status">' + records[i].status + '</span> ' + getOptions(records[i].status, records[i].id);
					result += '</div>';
		...
		$('tr').each(function() {
			if (!$(this).html().match('(<th>)')) {
				...
				if (records[count].status == 'Available for download') {
					records[count].typeClass = 'download';
					records[count].type = 'Download';
				}
		...
		$('#ill_requests .ill_requests').each(function() {
			...
			if (num === 0) {
				...
				switch (id) {
					case 'checkedout':
						message = 'No items checked out. Items appear here when they have been received physically by our Interlibrary Loan unit for processing or are on their way to your default delivery location. For all current holds and items checked out to you, log in to your Library account through SearchIt.';
						break;

					case 'available':
						message = 'No items available to download. Items will appear here when they are available for you to download from the web.';
						break;

					case 'inprocess':
						message = 'No items in process. Items will appear here after you make a new request and before it is available or checked out to you.';
						break;

					case 'past':
						message = "No past items. Items will appear here when you return items you've checked out."";
						break;

					case 'cancelled':
						message = 'No recently cancelled items.';
						break;
				}
