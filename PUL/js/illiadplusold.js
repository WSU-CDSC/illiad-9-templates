...
function illplus_requests() {
	...
	$(document).ready(function() {
		...
		function getNewStatus(status) {
			...
			var cleanStatus = status.replace("").trim();

			switch (cleanStatus) {
				case "Checked Out to Customer" :
                case "In Transit to Pickup Location" :
					newStatus = "On Its Way or Checked Out to You";
					break;
				...
				case "Delivered to Web" :
					newStatus = "Available to download";
					break;
                ...
                case "Potential Purchasing - Access Services" :
					newStatus = "Making Inquiries";
					break;
		...
		function appendSection(section) {
			...
			switch (section) {
				...
				case "available":
					title = "Available for Download"; break;
				case "checkedout":
					title = "On Its Way or Checked Out to You"; break;
				case "cancelled":
		...
		$("#ill_requests .ill_requests").each(function() {
			...
			if (num === 0) {
				...
				switch (id) {
					case "checkedout" :
						message = "No items checked out. Items appear here when they have been received physically by our Interlibrary Loan unit for processing or are on their way to your default delivery location. For all current holds and items checked out to you, log in to your Library account through SearchIt."; break;
					case "available" :
						message = "No items available to download. Items will appear here when they are available for you to download from the web."; break;
					case "inprocess" :
						message = "No items in process. Items will appear here after you make a new request and before it is available or checked out to you."; break;
					case "past" :
						message = "No past items. Items will appear here when you return items you've checked out."; break;
					case "cancelled" :
						message = "No recently cancelled items."; break;
				}
